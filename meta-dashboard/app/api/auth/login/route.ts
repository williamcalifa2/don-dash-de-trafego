...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => ({})) as { code?: unknown; email?: unknown; token?: unknown; c?: unknown; next?: unknown }
  ...
  const token = typeof body.token === 'string' ? body.token.trim().slice(0, 40) : ''
  const byEmail = !!email
